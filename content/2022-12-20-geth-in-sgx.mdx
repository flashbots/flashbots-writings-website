...
The initial sync process of the chain can take quite a long time, and currently requires up to 800GB of storage. We chose to copy the encrypted database, rather than run the sync within SGX.
...
On startup, the state DB is copied from the host system into the enclave, which also encrypts the data. This operation takes about 3 hours because of the data size and cryptographic operations.
