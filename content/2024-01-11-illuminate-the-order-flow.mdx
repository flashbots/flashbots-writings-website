...
We will show that defi's order flow landscape represents a nascent and fragile opportunity, showcasing a decentralized version of a robust order flow processing network using centralized infrastructure. We will argue that permissionless networks of searchers and liquidity provision can create competitive order execution, escaping tradfi’s reliance on a handful of sophisticated market makers to do so. We will argue that what defi needs to truly fulfill its vision and promise is to maintain the existing decentralization in this market, while also increasing competition and the ability to decentralize infrastructure at every layer of the stack.
...
In contrast to tradfi’s NBBO requirement for price improvement, which fails to account for optimal trade routing and misses private liquidity, defi’s version of an NBBO in solver systems is maintained by a competitive market of 22 solvers optimizing trade routes across [hundreds of public and private liquidity sources](https://help.1inch.io/en/articles/4585125-what-are-liquidity-sources-and-how-many-of-them-does-1inch-aggregate-from#h_cb89899f48).
...
### Decentralization and competition today
...
**Figure 14.** _Retail Trade Volume_ A sankey graph that displays the flow of transactions from frontend, solver auctions, solvers, order flow auctions, and finally to builders for inclusion.[⁴](#superscript-4)
