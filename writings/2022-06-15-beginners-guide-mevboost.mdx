...
`mev-geth` introduced a new RPC endpoint, `eth_sendBundle` - the message sent to this endpoint is called a bundle. **Bundles** consist of one or many transactions to be executed in an atomic batch.
...
### mev-boost
Mev-boost is an updated Flashbots architecture that is compatible with PoS Ethereum. The design is as follows: 
