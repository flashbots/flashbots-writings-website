...
**Tl;dr** [The first article of this series](/writings/order-flow-auctions-and-centralisation-I) outlined the threat that exclusive order flow (EOF) poses to the builder market. This article argues that order flow auctions can address part of the EOF problem and explores meaningful differences between two auction design types, while identifying similarities that point to a more fundamental problem.
<!--truncate-->
