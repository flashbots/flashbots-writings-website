...
title: The Anatomy of a Cryptopunk MEV Inspector
authors: [robert]
tags: [flashbots]
...
Last month I was pinged on Twitter about a novel form of MEV involving cryptopunks. In short, if a punk owner accepted a low bid on their punk then MEV bots would swoop in, bid marginally higher than the low bid, and snipe the punk from the low bidder. By doing so these bots were able to systematically buy cryptopunks at low prices at the expense of the users who had originally placed low bids! The punk community was upset.

With this in mind I turned to mev-inspect (“Inspect”) to try to shine a bright light on what was going on in the underbelly of the punk market. Inspect is Flashbots’ tool to inspect historical Ethereum blocks for MEV extraction, and the data pipeline underlying MEV-explore. Inspect attempts to classify MEV extraction by strategy and to surface relevant data for analysis. For example, it can spot arbitrage between a number of top DEXes as well as liquidations. It is the perfect tool to analyze new MEV strategies.

I wrote the 1st (Python) version of inspect, but it has since been replaced by a much, much more powerful foundation built by my colleagues Taarush, Gui, and Luke. The codebase had become 10x more professional than the original with 10x the features too. I was excited to try to contribute to Inspect by adding a new inspector for this punks use case.

My inspector is now live and in production, and I wanted to write this blog post up to help guide others in contributing to Inspect. This blog post documents the process of writing a new inspector and uses the cryptopunk snipe inspector as a reference.
...
To add a new MEV strategy, the cryptopunk snipe inspector, I took the following steps:
* Add the cryptopunk ABI [(link)](https://github.com/flashbots/mev-inspect-py/blob/main/mev_inspect/abis/cryptopunks/cryptopunks.json)
...
Here I implemented a new strategy inspector, which required all 7 steps. **But if you’re adding a new protocol (e.g. a DEX) to an existing inspector (e.g. arbitrage), then only the first 3 steps are necessary.**
...
For a given trace Inspect will attempt to classify it by looking to see:
...
We also add an entry under “Protocol” for the protocol we’re working on if it doesn’t already exist. Again, we simply add “[cryptopunks](https://github.com/flashbots/mev-inspect-py/blob/main/mev_inspect/schemas/traces.py#L49)” to the list.
...
Inspect works by building structured objects we call schemas. Each schema is stored in a separate file in the [schemas folder](https://github.com/flashbots/mev-inspect-py/tree/main/mev_inspect/schemas). They look like this:
...
class PunkBid(BaseModel):
    ...
    price: int
    ```
You will need to make a schema for each new object you’d like to create. In my case, I created three: punk bids, punk bid acceptances, and punk snipes. 
...
Congratulations! You’ve learned about the different moving parts involved in creating a new MEV inspector. I hope you’ll give writing one yourself a shot, and don’t be afraid to jump in the Flashbots Discord to ask questions in #mev-inspect. Come illuminate the dark forest with us, anon!

Thank you to Taarush, Chris, Gui, and Luke for reviewing this article.